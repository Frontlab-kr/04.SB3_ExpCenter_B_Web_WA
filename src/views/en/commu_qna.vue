<template>
  <div>
    <div class="d-flex title-xxlarge">
      <h1 class="title-xxlarge" style="position: relative;z-index: 10;background-color: white;">Q&A</h1>
      <v-spacer></v-spacer>
      <v-btn @click="ShowDialog()"><v-icon aria-hidden="true" style="margin-right: 6px;">mdi-pencil</v-icon>{{ $t("all_btn_write") }}</v-btn>
    </div>
    ...
    <div class="mt-2 pa-3">
      ...
      <v-data-table
        ...
        <template v-slot:default="props">
          <table class="nor_table_center">
            <caption id="table-caption" class="sr-only">Q&A table</caption>
            <thead>
              <tr>
      ...
      >
        ...
        <template v-slot:item.is_file="{ item, value }">
          <div v-if="value" @click="ClickIconDirect($event, item)" @keyup.enter="ClickIconDirect($event, item)" role="button" tabindex="0" aria-label="게시글에 첨부된 파일 다운로드 버튼 입니다."><v-icon aria-hidden="true">mdi-file</v-icon></div>
        </template>
      </v-data-table> -->
    </div>
    ...
    <v-dialog aria-modal="true" @update:modelValue="DialogClose" v-model="bDialog" width="80%">
      <v-card class="pa-4 pl-6 pr-6">
        <div class="d-flex">          
          ...
          <v-spacer></v-spacer>
          <v-btn variant="plain" class="mt-n1 mr-n3" icon @click="DialogClose" title="close"><v-icon aria-hidden="true">mdi-close</v-icon></v-btn>
        </div>

        <div style="max-height: 80vh; overflow: auto;">
          <v-row class="ma-0">
            <v-col cols="12" md="4" sm="12">
              <M4_Input v-model="oEditItem.CRT_USER_ID" readonly :label="$t('all_table_header_5')" :aria-label="$t('all_table_header_5')"></M4_Input>
            </v-col>
            <v-col cols="12">
              <M4_Input v-model="oEditItem.PST_TTL" :label="$t('all_table_header_4') + '(*)'" :aria-label="$t('all_table_header_4')"></M4_Input>
            </v-col>
            <v-col cols="12">
              <Editor v-model="oEditItem.PST_CN" @UpdateData="UpdateData" ref="FroalaEditor" aria-label="내용입력"></Editor>
            </v-col>
            <v-col cols="12" md="12" sm="12">
              <M4_Input v-model="oEditItem.FILE_NM" hide-details :label="$t('all_dialog_item_1')" :aria-label="$t('all_dialog_item_1')" @click:clear="onClear" clearable readonly></M4_Input>
            </v-col>
          </v-row>
        </div>
